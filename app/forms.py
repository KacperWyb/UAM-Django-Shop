...
class CheckoutForm(forms.ModelForm):
    class Meta:
        model = Order
        fields = ["zamówiony_przez", "adres_wysyłki",
                  "telefon", "email", "metoda_płatności"]


class CustomerRegistrationForm(forms.ModelForm):
    nazwa_użytkownika = forms.CharField(widget=forms.TextInput())
    hasło = forms.CharField(widget=forms.PasswordInput())
    email = forms.CharField(widget=forms.EmailInput())

    class Meta:
        model = Customer
        fields = ["nazwa_użytkownika", "hasło", "email", "imię_i_nazwisko", "adres"]

    def clean_username(self):
        uname = self.cleaned_data.get("nazwa_użytkownika")
        if User.objects.filter(username=uname).exists():
            raise forms.ValidationError(
                "Klient o tej nazwie użytkownika już istnieje.")
